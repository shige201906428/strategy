...
def fetch_japan_margin_data(ticker_code):
    """Yahoo!ファイナンスから日本株の信用売り残と信用倍率を取得する"""
    ...
    try:
        ...
        short_element = soup.find('th', text='売り残')
        short_val = "N/A"
        if short_element:
            td = short_element.find_next('td')
            if td:
                short_val = td.text.strip()
                
        ratio_element = soup.find('th', text='信用倍率')
        ratio_val = "N/A"
        if ratio_element:
            td = ratio_element.find_next('td')
            if td:
                ratio_val = td.text.strip() + " 倍"
                
        time.sleep(0.5)
        return short_val, ratio_val
    except Exception:
        return "N/A", "N/A"
        ...
        try:
            ...
            if is_bull_market:
                ...
                if signal_type:
                    ...
                    if ".T" in ticker_symbol:
                        short_volume, short_ratio = fetch_japan_margin_data(ticker_symbol)
                    else:
                        try:
                            t_info = yf.Ticker(ticker_symbol).info
                            shares_short = t_info.get('sharesShort')
                            pct_short = t_info.get('shortPercentOfFloat')
                            
                            if shares_short:
                                short_volume = f"{shares_short:,} 株"
                            if pct_short:
                                short_ratio = f"{round(pct_short * 100, 2)} %"
                        except Exception:
                            pass
...
def generate_html_report(df, output_path, title_suffix=""):
    """index.htmlとして概要・チャート・信用売り残情報・メモ欄付きレポートを出力する"""
    ...
    for idx, row in df.iterrows():
        ...
        <tr>
            ...
            <td class="vertical-middle" style="font-weight: 500; color: #c0392b;">{row['Short_Volume']}</td>
            <td class="vertical-middle"><small class="text-muted">{ratio_label}:</small> <br><strong>{row['Short_Ratio']}</strong></td>
            <td class="vertical-middle">
                <input type="text" class="form-control form-control-sm note-input" data-ticker="{ticker}" placeholder="メモを入力..." style="min-width: 150px;">
            </td>
        </tr>
        """
...
<head>
    ...
    <style>
        ...
        .container {{ margin-top: 30px; margin-bottom: 50px; width: 95%; max-width: 1500px; }}
        ...
        .note-input {{ border: 1px solid #ced4da; border-radius: 4px; }}
        .note-input:focus {{ border-color: #80bdff; box-shadow: 0 0 0 0.2rem rgba(0,123,255,.25); }}
...
<body>
    ...
    <script>
        $(document).ready(function() {
            // ローカルストレージから保存されたメモを復元する仕組み
            function loadNotes() {
                $('.note-input').each(function() {
                    var ticker = $(this).data('ticker');
                    var savedNote = localStorage.getItem('note_' + ticker);
                    if (savedNote) {
                        $(this).val(savedNote);
                    }
                });
            ...
            // メモが入力されたらブラウザのLocalStorageに自動保存
            $(document).on('input', '.note-input', function() {
                var ticker = $(this).data('ticker');
                var noteVal = $(this).val();
                localStorage.setItem('note_' + ticker, noteVal);
            });
...
if __name__ == "__main__":
    ...
    if result_df is None or result_df.empty:
        result_df = pd.DataFrame(columns=["Ticker", "Signal_Type", "Current_Price", "SMA_Value", "ST_Value", "Short_Volume", "Short_Ratio", "Note"])
...
# def generate_html_report(df, output_path, title_suffix=""):
#     """index.htmlとして概要・チャート・信用売り残情報付きレポートを出力する"""
#     jst_now = datetime.datetime.utcnow() + datetime.timedelta(hours=9)
#     now_str = jst_now.strftime("%Y-%m-%d %H:%M:%S (JST)")
...
#     <style>
#         body {{ background-color: #f8f9fa; color: #333; font-family: 'Helvetica Neue', Arial, sans-serif; }}
#         .container {{ margin-top: 30px; margin-bottom: 50px; width: 95%; max-width: 1400px; }}
#         .header-section {{ background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); color: white; padding: 30px; border-radius: 8px; margin-bottom: 30px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }}
#         .card {{ border: none; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border-radius: 8px; }}
#         .table thead th {{ background-color: #2c3e50; color: white; border: none; vertical-align: middle; }}
#         .vertical-middle {{ vertical-align: middle !important; }}
...
#     if result_df is None or result_df.empty:
#         result_df = pd.DataFrame(columns=["Ticker", "Signal_Type", "Current_Price", "SMA_Value", "ST_Value", "Short_Volume", "Short_Ratio"])
